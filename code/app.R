...
server <- function(input, output) {
  ...
  # Network diagram function can shift to construction.R eventually
  activity_on_node_plot_plotly <- function(data) {
    # Edge processing
    edges <- c()
    for (i in 1:nrow(data)) {
      activity <- as.character(data$Activity[i])  # Ensure activity is a character
      predecessors <- unlist(strsplit(as.character(data$Predecessor[i]), ','))
      
      # For checking
      #cat("Processing Job:", activity, "with Predecessors:", predecessors, "\n")
      
      for (predecessor in predecessors) {
        if (!is.na(predecessor) & predecessor != '-' & predecessor != '') {
          edges <- append(edges, c(predecessor, activity))
        }
      }
    }
    
    # Ensure all unique job names are used as vertices
    all_activities <- unique(c(data$Activity, unlist(strsplit(paste(data$Predecessor, collapse=","), ","))))
    all_activities <- unique(all_activities[all_activities != "" & all_activities != "-"])
    
    # Create the igraph object
    g1 <- graph(edges = edges, directed = TRUE)
    
    # Add any missing vertices
    existing_vertices <- V(g1)$name
    missing_vertices <- setdiff(all_activities, existing_vertices)
    g1 <- add_vertices(g1, length(missing_vertices), name = missing_vertices)
    
    # Check if root exists in the graph
    if ("A" %in% V(g1)$name) {
      root_node <- "A"
    } else {
      root_node <- V(g1)$name[1]  # Use the first vertex as root if "A" is not present
    }
    
    # Layout calculation
    set.seed(1)  # For consistent layout
    if (vcount(g1) <= 20) {
      layout <- layout_as_tree(g1, root = root_node)
    } else if (vcount(g1) <= 50) {
      layout <- layout_with_fr(g1)
    } else {
      layout <- layout_with_kk(g1)
    }
    layout <- norm_coords(layout, -1, 1, -1, 1) * 1.5
    
    # Convert to data frame for Plotly
    layout_df <- as.data.frame(layout)
    rownames(layout_df) <- names(V(g1))
    
    # Get edgelist for annotations
    if (length(E(g1)) > 0) {
      edgelist <- get.edgelist(g1)
      
      # Start and end coordinates for annotations
      x0 <- layout_df[edgelist[,1], 1] 
      y0 <- layout_df[edgelist[,1], 2]
      x1 <- layout_df[edgelist[,2], 1]
      y1 <- layout_df[edgelist[,2], 2]
      
      # Adjust arrow positions for better visualization
      arrow_length <- 0.1  # Adjust the arrow length as needed (0 to 1)
      x_adj <- (x1 - x0) * arrow_length
      y_adj <- (y1 - y0) * arrow_length
      
    } else {
      x0 <- y0 <- x1 <- y1 <- x_adj <- y_adj <- numeric(0)
    }
    ...
    p <- plot_ly(
      ...
      marker = list(size = 15, color = "lightblue")
    )
    
    # Add arrows using add_annotations
    if (length(E(g1)) > 0) {
    ...
    p %>% layout(
      title = "Activity-on-Node (AON) Diagram",
  ...
  output$cpm_network_plot <- renderUI({
    if (is.null(input$file) & input$dataInputType=='file') {
      ...
      renderPlotly({
        activity_on_node_plot_plotly(cpm_processed_data())
      }) 
    }
  })
